...
public class ClientListener extends Thread {
    ...
    public ClientListener(Socket socket, Long id, UsersService usersService,
                          MessageService messageService) throws IOException,IllegalArgumentException {
        String command;
        this.socket = socket;
        this.id = id;
        ...
        if (!userService.signIn(login)) {
            errorExit("Not found user with this login!");
        }
        send("Enter password:");
    ...
    public void run() {
        ...
        try {
            while (true) {
                ...
                System.out.println("Client " + id + " - \"" + login + "\" say = \"" + clientMessage + "\"");
                messageService.saveMessage(login, clientMessage);

                for (ClientListener cl : Server.serverList) {
                    cl.send(login + ": " + clientMessage);
                }
            }
        } catch (IOException e) {
        }
